...
void	put_error(t_asm *asemb, int err_n, int row, int col)
{
	if (err_n == ERR_LEX)
		...
		printf("Lexical error - unexpected end of input"\
			" (Perhaps you forgot to end with a newline ?\n");
	else if (err_n == ERR_SYNTX)
		printf("Syntax error at [%03u:%03i]\n", row, col);
	else if (err_n == ERR_LONG_NAME)
		printf("Champion name too long (Max length %u)\n", PROG_NAME_LENGTH);
	else if (err_n == ERR_LONG_COM)
		printf("Champion comment too long"\
			" (Max length %u)\n", COMMENT_LENGTH);
	else
...
{
	printf("Invalid parameter %i type indirect"\
...
{
	printf("Undeclared label \"%s\" in line %i\n", label, row);
...
void	put_error_file(t_asm *asemb, int err_n, char *file_name)
{
	if (err_n == ERR_NUMB_ARG)
		printf("incorrect number of arguments - expected \"1\" arg\n");
	else if (err_n == ERR_READ_FILE)
		printf("Can't read input file \"%s\"\n", file_name);
	else if (err_n == ERR_FILE_NAME)
		printf("Invalid input file name - use \"exemple.s\"\n");
